		...
		for(int i = 0;i < mWeibo2smsList.size();i++){
			...
			if(w.GetUserScreenName().equals(w2s.weiboId) || w.GetUserName().equals(w2s.weiboId)){
				...
				try{
					...
					//RRR:包括图片连接 : String text = w.getShareSMSContain(false,false);参数:前面控制图片，后面控制原文转发
					String text = w.getShareSMSContain(false,false);
					...
					*/
					...
					m_mainApp.SetErrorString("w2s send succ:"+"[ Total:"+ (text.length() + 10)+"]" +"&" + w2s.weiboId + "->" + w2s.smsPhone  );
