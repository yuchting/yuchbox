...
public class ChatField extends Manager implements FocusChangeListener{
	...
	public final static int	fsm_ownChatTextBGColor		= 0xbbe4fe; //RRR:对话字体背景 0x95be0d;
	public final static int	fsm_otherChatTextBGColor	= 0xededff; //RRR: 对话字体背景0xd0d1d3;
	...
	final static	Font	fsm_timeFont		= MainIMScreen.fsm_defaultFont.derive(MainIMScreen.fsm_defaultFont.getStyle(),12);//RRR:320*240
...
*/
	...
	protected void subpaint(Graphics _g){
		...
		int t_time_x = 0;
		int t_time_y = getPreferredHeight() - fsm_timeFont.getHeight() ;
		...
		if(recvMain.sm_imDisplayTime){
			...
			try{
				...
				_g.fillRoundRect(t_time_x,t_time_y, t_time_width, fsm_timeFont.getHeight(), 0, 0);
				
//				_g.setColor(fsm_timeTextBorderColor);//RRR:去除时间边框
				_g.drawRoundRect(t_time_x,t_time_y, t_time_width, fsm_timeFont.getHeight(), 0, 0);
